...
OUTPUT_DIR     = "/tmp/Reportes/Naturisa"
...
COLUMNAS_ORDEN = ["Ingreso a Muro","Saldo Muro","Cargado Tolva","Sobrante Tolva","Consumo Voleo","Saldo Tolva"]
...
def obtener_datos(fecha: str, token: str) -> list:
    ...
    for key in ("data","records","items","result","content"):
        if key in data and isinstance(data[key], list):
            return data[key]
    with open("/tmp/respuesta_debug.json","w") as f:
    ...
    try:
        ...
        for i, (_, row) in enumerate(df_alertas.iterrows()):
            ...
            <tr style="background:{bg};">
                <td style="padding:7px 14px; border:1px solid #ccc; font-weight:500;">{row['PSC']}</td>
                <td style="padding:7px 14px; border:1px solid #ccc; text-align:center;">{int(row.get('Ingreso a Muro', 0))}</td>
                <td style="padding:7px 14px; border:1px solid #ccc; text-align:center;">{int(row.get('Saldo Muro', 0))}</td>
                <td style="padding:7px 14px; border:1px solid #ccc; text-align:center;">{int(row.get('Cargado Tolva', 0))}</td>
                <td style="padding:7px 14px; border:1px solid #ccc; text-align:center;">{int(row.get('Sobrante Tolva', 0))}</td>
                <td style="padding:7px 14px; border:1px solid #ccc; text-align:center;">{int(row.get('Consumo Voleo', 0))}</td>
                <td style="padding:7px 14px; border:1px solid #ccc; text-align:center; font-weight:bold; color:#8B1A1A;">{int(row.get('Saldo Tolva', 0))}</td>
            </tr>"""
        cuerpo = f"""
<div style="font-family:Arial, sans-serif; max-width:820px; margin:0;">
  <div style="background:#1a1a2e; padding:18px 24px; border-radius:6px 6px 0 0;">
    <h2 style="color:#FFFFFF; margin:0; font-size:16px; letter-spacing:1px;">
      AP1 PRO &nbsp;|&nbsp; PISCINAS CON SALDO TOLVA SIN MOVIMIENTO
    </h2>
    <p style="color:#aaa; margin:4px 0 0; font-size:12px;">Fecha: {fecha} &nbsp;|&nbsp; Total piscinas en alerta: <b style="color:#fff;">{n_alertas}</b></p>
  </div>
  <table style="width:100%; border-collapse:collapse; font-size:13px; border:1px solid #ccc; border-top:none;">
    <thead>
      <tr style="background:#2c2c2c; color:#FFFFFF;">
        <th style="padding:9px 14px; border:1px solid #555; text-align:left;">PSC</th>
        <th style="padding:9px 14px; border:1px solid #555; text-align:center;">Ingreso a Muro</th>
        <th style="padding:9px 14px; border:1px solid #555; text-align:center;">Saldo Muro</th>
        <th style="padding:9px 14px; border:1px solid #555; text-align:center;">Cargado Tolva</th>
        <th style="padding:9px 14px; border:1px solid #555; text-align:center;">Sobrante Tolva</th>
        <th style="padding:9px 14px; border:1px solid #555; text-align:center;">Consumo Voleo</th>
        <th style="padding:9px 14px; border:1px solid #555; text-align:center;">Saldo Tolva</th>
      </tr>
    </thead>
    <tbody>
      {filas_html}
  ...
  <p style="font-size:11px; color:#999; margin-top:12px; text-align:left;">
    Este correo fue generado automaticamente por el sistema de reportes Naturisa.
        ...
        print(f"[{datetime.now():%H:%M:%S}] Correo enviado correctamente")
    except Exception as e:
